...
namespace hnswlib {
    ...
    static float
    WeightedInnerProduct(const void *pVect1, const void *pVect2, const void *pWVectv, const void *qty_ptr) {
        size_t qty = *((size_t *) qty_ptr);
        float res = 0;
        for (unsigned i = 0; i < qty; i++) {
            res += ((float *) pVect1)[i] * ((float *) pVect2)[i] * ((float *) pWVectv)[i];
        }
        return (1.0f - res);
    ...
    public:
        InnerProductSpace(size_t dim) {
            ...
            fstweighteddistfunc_ = WeightedInnerProduct;
            dim_ = dim;
